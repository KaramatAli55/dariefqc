...
function custom_display_lines($focus, $field, $value, $view){
    ...
    if($view == 'EditView'){
        ...
        $html .= '<input type="hidden" name="vathidden" id="vathidden" value="'.get_select_options_with_id($app_list_strings['vat_list'], '').'">
                  <input type="hidden" name="psthidden" id="psthidden" value="'.get_select_options_with_id($app_list_strings['pst_list'], '').'">
        ...
        if($focus->id != '') {
            ...
            while ($row = $focus->db->fetchByAssoc($result)) {
                ...
                $GLOBALS['log']->fatal("LineItem: ". print_r($line_item, 1));
        ...
        $html .= "<table style='table-layout: fixed;' border='0' width='100%' cellpadding='0' cellspacing='0'>";
        ...
        while ($row = $focus->db->fetchByAssoc($result)) {
            ...
            if($enable_groups && ($group_id != $row['group_id'] || $i == 0)){
                ...
                $groupEnd .= "</tr>";
                $groupEnd .= "<tr>";
                $groupEnd .= "<td class='tabDetailViewDL' colspan='10' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_PST_TAX_AMOUNT'].":&nbsp;&nbsp;</td>";
                $groupEnd .= "<td class='tabDetailViewDL' style='text-align: right;padding:2px;'>".currency_format_number($group_item->pst_tax_amount, $params)."</td>";
                $groupEnd .= "</tr>";
                $groupEnd .= "<tr>";
            ...
            if($line_item->product_id != '0' && $line_item->product_id != null){
                if($productCount == 0)
                {
                    ...
                    $product .= "<td width='9%' class='tabDetailViewDL' style='text-align: left;padding:2px;' scope='row'>".$mod_strings['LBL_PRODUCT_QUANITY']."</td>";
                    $product .= "<td width='10%' class='tabDetailViewDL' style='text-align: left;padding:2px;' scope='row'>".$mod_strings['LBL_PRODUCT_NAME']."</td>";
                    $product .= "<td width='10%' class='tabDetailViewDL' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_COST_PRICE']."</td>";
                    $product .= "<td width='4%' class='tabDetailViewDL' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_MARKUP_AMT']."</td>";
                    ...
                    $product .= "<td width='11%' class='tabDetailViewDL' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_PST_TAX_AMOUNT']."</td>";
                    $product .= "<td width='12%' class='tabDetailViewDL' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_PST_AMT']."</td>";
                ...
                $product .= "<td class='tabDetailViewDF' style='padding:2px;'>".stripDecimalPointsAndTrailingZeroesCustom(format_number($line_item->product_qty),$sep[1])."</td>";
                ...
                $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".(int)$line_item->product_markup."%</td>";
                ...
                if($locale->getPrecision()){
                    $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".rtrim(rtrim(format_number($line_item->pst), '0'),$sep[1])."%</td>";
                } else {
                    $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".format_number($line_item->pst)."%</td>";
                }
                $product .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".currency_format_number($line_item->pst_amt,$params )."</td>";
                ...
                if($serviceCount == 0)
                {
                    ...
                    $service .= "<td colspan='3' width='12%' class='dataLabel' style='text-align: right;padding:2px;' scope='row'>".$mod_strings['LBL_TOTAL_PRICE']."</td>";
                    $service .= "</tr>";
                }
                ...
                $service .= "<td class='tabDetailViewDF' style='text-align: right; padding:2px;'>".get_discount_string_custom($line_item->discount, $line_item->product_discount, $params, $locale, $sep)."</td>";
                ...
                $service .= "<td colspan='3' class='tabDetailViewDF' style='text-align: right; padding:2px;'>".currency_format_number($line_item->product_total_price,$params )."</td>";
                $service .= "</tr>";
...
function stripDecimalPointsAndTrailingZeroesCustom($inputString, $decimalSeparator)
{
    return preg_replace('/'.preg_quote($decimalSeparator).'[0]+$/','',$inputString);
}

function get_discount_string_custom($type, $amount, $params, $locale, $sep){
...
function display_shipping_vat_custom($focus, $field, $value, $view){
