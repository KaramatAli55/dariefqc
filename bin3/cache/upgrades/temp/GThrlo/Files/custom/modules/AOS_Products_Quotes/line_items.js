...
function insertLineItems(product,group){
    ...
    for(var p in product){
        if(document.getElementById(type + p + ln) !== null){
            if(product[p] !== '' && isNumeric(product[p]) && p != 'vat' && p != 'pst' && p != 'product_markup' && p != 'product_id' && p != 'name' && p != "part_number"){
...
function insertProductLine(tableid, groupid) {
    ...
    var pst_hidden = document.getElementById("psthidden").value;
    ...
    c.innerHTML = "<input type='text' name='product_product_cost_price[" + prodln + "]' id='product_product_cost_price" + prodln + "' maxlength='50' value='' title='' tabindex='116' onblur='calculateLine(" + prodln + ",\"product_\");' class='product_list_price'>";
    ...
    d.innerHTML = "<input type='text' name='product_product_markup[" + prodln + "]' id='product_product_markup" + prodln + "'  maxlength='50' value='' title='' tabindex='116' onblur='calculateLine(" + prodln + ",\"product_\");' onblur='calculateLine(" + prodln + ",\"product_\");' class='product_discount_text'>";
//<input type='hidden' name='product_product_markup_amount[" + prodln + "]' id='product_product_markup_amount" + prodln + "' value=''  />'';
    ...
    g.innerHTML = "<input type='text' name='product_pst_amt[" + prodln + "]' id='product_pst_amt" + prodln + "' maxlength='250' value='' title='' tabindex='116' readonly='readonly' class='product_vat_amt_text'>";
    g.innerHTML += "<select tabindex='116' name='product_pst[" + prodln + "]' id='product_pst" + prodln + "' onchange='calculateLine(" + prodln + ",\"product_\");' class='product_vat_amt_select'>" + pst_hidden + "</select>";

    if (typeof currencyFields !== 'undefined'){
        currencyFields.push("product_vat_amt" + prodln);
        currencyFields.push("product_pst_amt" + prodln);
    }
    var h = x.insertCell(9);
    h.innerHTML = "<input type='text' name='product_product_total_price[" + prodln + "]' id='product_product_total_price" + prodln + "' maxlength='50' value='' title='' tabindex='116' readonly='readonly' class='product_total_price'><input type='hidden' name='product_group_number[" + prodln + "]' id='product_group_number" + prodln + "' value='"+groupid+"'>";
    ...
    addToValidate('EditView','product_product_markup'+prodln,'int',true,"Markup must be number.");
...
function formatListPrice(ln){
    ...
    if (typeof currencyFields !== 'undefined'){
        ...
        //var dollar_product_price = ConvertToDollar(document.getElementById('product_product_cost_price' + ln).value, product_currency_rate);
        //document.getElementById('product_product_cost_price' + ln).value = format2Number(ConvertFromDollar(dollar_product_price, lastRate));
...
function insertProductHeader(tableid){
    ...
    c.innerHTML=SUGAR.language.get(module_sugar_grp1, 'LBL_COST_PRICE');

    var d=x.insertCell(4);
    d.style.color="rgb(68,68,68)";
    d.innerHTML=SUGAR.language.get(module_sugar_grp1, 'LBL_MARKUP_AMT');
    ...
    g.innerHTML=SUGAR.language.get(module_sugar_grp1, 'LBL_PST_AMT');

    var h=x.insertCell(8);
    h.style.color="rgb(68,68,68)";
    h.innerHTML=SUGAR.language.get(module_sugar_grp1, 'LBL_TOTAL_PRICE');
...
function insertGroup()
{
    ...
    if(enable_groups){
        ...
        footer_cell6.innerHTML="<span class='totals'><label>"+SUGAR.language.get(module_sugar_grp1, 'LBL_PST_TAX_AMOUNT')+":</label><input name='group_pst_tax_amount[]' id='"+ table.id +"pst_tax_amount' class='group_pst_tax_amount' maxlength='26' value='' title='' tabindex='120' type='text' readonly></span>";
        ...
        footer_cell7.innerHTML="<span class='totals'><label>"+SUGAR.language.get(module_sugar_grp1, 'LBL_GROUP_TOTAL')+":</label><input name='group_total_amount[]' id='"+ table.id +"total_amount' class='group_total_amount'  maxlength='26' value='' title='' tabindex='120' type='text' readonly></span>";
        ...
        if (typeof currencyFields !== 'undefined'){
            ...
            currencyFields.push("" + table.id+ 'pst_tax_amount');
...
function calculateLine(ln, key){
    ...
    if(key == "product_" && document.getElementById(key + 'product_cost_price' + ln) !== null && document.getElementById(key + 'product_markup' + ln) !== null){
        var costPrice = get_value(key + 'product_cost_price' + ln);
        var markup = get_value(key + 'product_markup' + ln);
        ...
        markup = (markup/100) * costPrice;
        productUnitPrice = costPrice + markup;
        document.getElementById(key + 'product_unit_price' + ln).value = format2Number(costPrice + markup);
        /*}
         else
         {
        ...
        document.getElementById(key + 'product_cost_price' + ln).value = format2Number(costPrice);
        ...
        //document.getElementById(key + 'product_markup' + ln).value = format2Number(markup, 6);
    }
    else if(key == "service_" && document.getElementById(key + 'product_list_price' + ln) !== null && document.getElementById(key + 'product_discount' + ln) !== null){
    ...
    if(key === "product_") {
        var pst = unformatNumber(document.getElementById(key + 'pst' + ln).value, ',', '.');
        var totalpst=(productTotalPrice * pst) /100;
        document.getElementById(key + 'pst_amt' + ln).value = format2Number(totalpst);
    }
...
function calculateTotal(key)
{
    ...
    for (i=0; i < length; i++) {
        ...
        for (j=0; j < input.length; j++) {
            ...
            if (input[j].id.indexOf('product_markup_amount') != -1)
            {
                markup_amt = unformat2Number(input[j].value);
            }
            if (input[j].id.indexOf('vat_amt') != -1)
            {
                product_vat_amt = unformat2Number(input[j].value);
            }
            if (input[j].id.indexOf('pst_amt') != -1)
            {
                product_pst_amt = unformat2Number(input[j].value);
            }
        ...
        if (product_pst_amt !== 0 && deleted != 1) {
            pst_tax += product_pst_amt;
        }
    }
    ...
    pst_tax = eval(pst_tax);

    subtotal = tot_amt + dis_tot + markup_amt;
    ...
    set_value(key+'pst_tax_amount', pst_tax);

    set_value('tax_amount', tax);

    set_value(key+'subtotal_tax_amount',subtotal + tax);
    var total_amount_ = subtotal + tax + shipping;
    total_amount_ = parseFloat(total_amount_);
    total_amount_ += pst_tax;
    set_value(key+'total_amount', total_amount_);
}
